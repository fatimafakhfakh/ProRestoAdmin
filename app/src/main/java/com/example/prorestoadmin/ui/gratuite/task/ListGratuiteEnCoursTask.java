...
public class ListGratuiteEnCoursTask extends AsyncTask<String, String, String> {

    String res;

    Activity activity;

    String codeRecouvreur;
    Date date_jour;
    int mode_gratuite ;
    ...
    public ListGratuiteEnCoursTask(Activity activity, String codeRecouvreur, Date date_jour, int mode_gratuite  , ExpandableListView elv_list_gratuite_en_cours, ProgressBar progressBar) {
        this.activity = activity;
        this.codeRecouvreur = codeRecouvreur;
        this.date_jour = date_jour;
        this.mode_gratuite=mode_gratuite ;
        this.elv_list_gratuite_en_cours = elv_list_gratuite_en_cours;
    ...
    protected String doInBackground(String... strings) {
        ...
        try {
            ...
            if (con == null) {
                ...
                if (codeRecouvreur.equals("")) {
                    CONDITION = CONDITION + " ";
                } else {
                    CONDITION = CONDITION + " and    CodeRepresentant = '" + codeRecouvreur + "' ";
                }
                ...
                String query = "    select Client. CodeClient , Client.RaisonSociale , DateDebut  ,DateFin  from Vue_MaquetteBonGratuite\n" +
                        ...
                        "    where  1=1  " + CONDITION + "    and   '" + sdf.format(date_jour) + "'   between DateDebut and DateFin \n" +
                ...
                while (rs.next()) {
                    ...
                    else  if (mode_gratuite== 2 ) // gratuite  proche à atteindre
                    {
                        if (gratuiteClient.isGratuit_proche_atteint())
                            listGratuiteClient.add(gratuiteClient);
                    }

                    else  if (mode_gratuite== 3 ) // gratuite  non atteint
                    {
                        if (!gratuiteClient.isGratuit_atteint()  && !gratuiteClient.isGratuit_proche_atteint() )
                            listGratuiteClient.add(gratuiteClient);
                    }
    ...
    protected void onPostExecute(String s) {
        ...
        elv_list_gratuite_en_cours.setAdapter(adapter);
